...
exports.main_handler = async (event, context, callback) => {
  try {
    ...
    for (const v of event["Message"].split("&")) {
      ...
      //request('https://gitee.com/'+v+'.js', function (error, response, body) {
      // eval(response.body)
      //})

      //3.执行github远端的js文件(因github的raw类型的文件被墙,此方法云函数不推荐)
      request('https://raw.githubusercontent.com/shuye73/MyActions/main/' + v + '.js', function (error, response, body) {
        eval(response.body)
      })
    }
  } catch (e) {
